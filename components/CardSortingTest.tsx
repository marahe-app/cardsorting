...
export const CardSortingTest: React.FC<CardSortingTestProps> = ({ task, onSubmit, onBack }) => {
    ...
    const UnsortedCardsColumn = () => (
        <div
            ...
            className={`bg-gray-800/50 p-4 rounded-xl transition-all h-full ${dragOverCategory === 'unsorted' ? 'bg-indigo-900/50 ring-2 ring-indigo-500' : ''}`}
        >
            <h2 className="text-xl font-semibold text-white mb-3 sticky top-0 bg-gray-800/50 py-2">Tarjetas sin Clasificar ({unsortedCards.length})</h2>
            <div className="space-y-3 lg:max-h-[65vh] lg:overflow-y-auto">
                    ...
                    {unsortedCards.map(card => {
                        ...
                        return (
                             <div 
                                ...
                                className={`bg-gray-700 p-3 rounded-md cursor-grab active:cursor-grabbing shadow-md text-gray-200 w-48 flex-shrink-0 transition-opacity ${isDraggingThisCard && isTouchDrag ? 'opacity-30' : 'opacity-100'}`}
                            >
                                {card.content}
                            </div>
                        )
                    ...
                    {unsortedCards.map(card => {
                        ...
                        return (
                            <div 
                                ...
                                onDragStart={() => { setIsTouchDrag(false); handleDragStart(card); }}
                                onTouchStart={(e) => handleTouchStart(e, card)}
                                className={`bg-gray-700 p-3 rounded-md cursor-grab active:cursor-grabbing shadow-md text-gray-200 w-full transition-opacity ${isDraggingThisCard && isTouchDrag ? 'opacity-30' : 'opacity-100'}`}
                            >
                                {card.content}
                            </div>
                        )
                ...
                {unsortedCards.length === 0 && (
                    <div className="w-full flex-grow flex items-center justify-center text-gray-400 p-4">
        ...
        <div className="container mx-auto max-w-7xl py-10 px-4 space-y-8">
            ...
            {ghost && (
              <div 
                ...
                className="bg-indigo-600 p-3 rounded-md shadow-2xl text-white w-48 ring-2 ring-white/50"
              >
                {ghost.card.content}
              </div>
            )}
            ...
            <header className="bg-gray-800 p-6 rounded-xl shadow-lg">
            ...
            <div className="bg-gray-700/50 p-5 rounded-lg border border-gray-600">
                ...
                >
                    <h3 className="text-xl font-semibold text-white">Instrucciones Detalladas</h3>
                    <svg
                        xmlns="http://www.w3.org/2000/svg"
                        className={`h-6 w-6 text-gray-400 transition-transform transform ${isInstructionsOpen ? 'rotate-180' : ''}`}
                        fill="none"
                        viewBox="0 0 24 24"
                        stroke="currentColor"
                        aria-hidden="true"
                ...
                {isInstructionsOpen && (
                    <div id="instructions-content" className="text-gray-300 mt-4 space-y-2">
                ...
                <div className="lg:col-span-2 space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {categories.map(category => (
                                ...
                                <div className="flex justify-between items-center gap-2">
                                     {editingCategoryId === category.id ? (
                                        <input
                                            ...
                                            className="flex-grow p-1 bg-gray-600 rounded-md border border-indigo-500 focus:ring-indigo-500 focus:border-indigo-500 text-lg text-indigo-300 w-full"
                                            autoFocus
                                        />
                                    ) : (
                                        <h3 className="text-lg font-bold text-indigo-300 flex-grow truncate" title={category.name}>{category.name}</h3>
                                    )}
                                    <div className="flex-shrink-0 flex items-center">
                                        <button 
                                            ...
                                            className="text-gray-500 hover:text-indigo-400 p-1 rounded-full transition-colors"
                                        >
                                           {editingCategoryId === category.id ? (
                                        ...
                                        <button 
                                            ...
                                            className="flex-shrink-0 text-gray-500 hover:text-red-400 p-1 rounded-full transition-colors"
                                        >
                                            <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                                    ...
                                    {category.cards.map(card => {
                                         ...
                                         return (
                                             <div 
                                                 ...
                                                 onDragStart={() => { setIsTouchDrag(false); handleDragStart(card); }} 
                                                 onTouchStart={(e) => handleTouchStart(e, card)}
                                                 className={`bg-gray-700 p-3 rounded-md cursor-grab active:cursor-grabbing shadow-md text-gray-200 transition-opacity ${isDraggingThisCard && isTouchDrag ? 'opacity-30' : 'opacity-100'}`}
                                             >
                                                 {card.content}
                                             </div>
                                         );
                         ...
                         {categories.length === 0 && (
                            <div className="md:col-span-2 flex items-center justify-center text-center p-10 bg-gray-800/50 rounded-xl border-2 border-dashed border-gray-700">
                                <p className="text-gray-400">Crea tu primera categoría para empezar a organizar las tarjetas.</p>
                            </div>
                        )}
                    </div>
                </div>
